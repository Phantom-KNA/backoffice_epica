...
namespace Epica.Web.Operacion.Controllers
{
    ...
    public class TransaccionesController : Controller
    {
        ...
        public async Task<JsonResult> Consulta(List<RequestListFilters> filters, string idAccount = "")
        {
            ...
            foreach (var row in ListPF)
            {
                ...
                List.Add(new ResumenTransaccionResponseGrid
                {
                    ...
                    nombreBeneficiario = !string.IsNullOrWhiteSpace(row.nombreBeneficiario) ? row.nombreBeneficiario: "-",
                    monto = row.monto,
                    estatus = row.estatus,
                    ...
                    alias = row.alias,
                    clabeCobranza = !string.IsNullOrWhiteSpace(row.clabeCobranza) ? row.clabeCobranza : "-",
                    cuetaOrigenOrdenante = !string.IsNullOrWhiteSpace(row.cuetaOrigenOrdenante) ? row.cuetaOrigenOrdenante : "-",
                    descripcionMedioPago = !string.IsNullOrWhiteSpace(row.descripcionMedioPago) ? row.descripcionMedioPago : "-",
        ...
        {
            var loginResponse = _userContextService.GetLoginResponse();
            var validacion = loginResponse?.AccionesPorModulo.Any(modulo => modulo.ModuloAcceso == "Operacion" && modulo.Ver == 0);
            if (validacion == true)
            {
                return View(loginResponse);
            }
